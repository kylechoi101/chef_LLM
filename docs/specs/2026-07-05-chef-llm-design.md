...
**Storage playbook (verified on-cluster 2026-07-14).** Pod mount map: pod-default `$HOME` = the *workspace* volume (10 GB quota, shared with the group — never write chef_LLM data there); the real 100 GB home is mounted at **`~/private`** inside pods (= `~` on dsmlp-login) — all project work lives under `~/private/Chef_LLM`; `/scratch` = node-local disk, ephemeral per pod (27 GB free on CPU nodes; probe the GPU node before relying on it); `/datasets-3` = 80 TB shared NFS (staff-managed; request hosting via datahub@ucsd.edu if ever needed).

Checkpoint arithmetic that drives the plan: 8B weights (bf16) = 16 GB, but a resumable full-training checkpoint (weights + AdamW state) ≈ 112 GB — **more than the whole home quota**. Therefore, for Phase 2:
- **Run state on `/scratch`** (fast, big on GPU nodes), **rotating weights-only checkpoints (16 GB, keep ≤3) to `~/private`**, so preemption costs at most re-warming the optimizer.
- **8-bit optimizer (AdamW8bit)** cuts persisted full state to ~48–56 GB — fits home if a full-state save is wanted.
- **Durable weights live on a private HuggingFace Hub repo** (free, no 100 GB ceiling, survives loss of DSMLP access entirely) — same role GitHub plays for code.
- Before the full CPT run: email **rcd-support@ucsd.edu** for a temporary home-quota bump (documented, routine) and for batch-job time-limit extensions. Long runs use the launcher's batch flags (`-b` background / `-B` queue-only, `-f` run-and-dump) with checkpoint-resume mandatory — `-p normal` for real runs, `-p low` is preemptible.
- Phase 1 is unaffected: critic checkpoints are 0.4–4 GB; everything fits in home with room to spare.
